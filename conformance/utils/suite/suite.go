...
func (suite *ConformanceTestSuite) Setup(t *testing.T, tests []ConformanceTest) {
	...
	if supportsGateway {
		...
		caConfigMap, ca, caPrivKey := kubernetes.MustCreateCASignedCertConfigMap(t, "gateway-conformance-infra", "ca-certificate", []string{"abc.example.com"})
		suite.Applier.MustApplyObjectsWithCleanup(t, suite.Client, suite.TimeoutConfig, []client.Object{caConfigMap}, suite.Cleanup)
		secret = kubernetes.MustCreateCertSecretWithCA(t, "gateway-conformance-infra", "tls-checks-certificate", []string{"abc.example.com"}, ca, caPrivKey)
		suite.Applier.MustApplyObjectsWithCleanup(t, suite.Client, suite.TimeoutConfig, []client.Object{secret}, suite.Cleanup)
